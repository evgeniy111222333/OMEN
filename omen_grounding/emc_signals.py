...
def grounding_emc_features(metadata: Mapping[str, Any] | None) -> Dict[str, float]:
    ...
    grounding_memory_signal = max(memory_grounding, trace_grounding, interlingua_grounding)
    ...
    recall_readiness = _clip01(uncertainty * grounding_memory_signal)
    ...
    verification_pressure = _clip01(
        ...
        + (0.20 * deferred_ratio)
        + (0.15 * (1.0 - mean_confidence))
        + (0.20 * verification_repair)
        + (0.10 * conflict_pressure)
        + (0.20 * world_state_contradiction)
    )
    abduction_pressure = _clip01(
        (verification_pressure * (1.0 - 0.5 * recall_readiness))
        + (0.35 * hidden_cause_pressure)
        + (0.20 * world_state_contradiction)
